...
public:
  ...
  void get_entries_common(cxt_id_t cxt_id, const std::string &table_name,
    ...
    StatefulTable *st = switch_->get_context(0)->get_stateful_table(table_name);
    ...
    for (const auto &entry : entries) {
      ...
      if (st) {
        e.__set_flow_registers(st->dump_flow_context(entry.handle));
      }
  ...
  void bm_mt_get_entries(std::vector<BmMtEntry> & _return, const int32_t cxt_id, const std::string& table_name) {
    ...
    switch (switch_->mt_get_type(cxt_id, table_name)) {
      case MatchTableType::NONE:
        return;
      case MatchTableType::STATEFUL:
        get_entries_common<FlowTable, &RuntimeInterface::mt_st_get_entries>(
  ...
  void bm_mt_get_entry(BmMtEntry& _return, const int32_t cxt_id, const std::string& table_name, const BmEntryHandle entry_handle) {
    ...
    switch (switch_->mt_get_type(cxt_id, table_name)) {
      case MatchTableType::NONE:
        return;
      case MatchTableType::STATEFUL:
        get_entry_common<FlowTable, &RuntimeInterface::mt_st_get_entry>(
    ...
    switch (switch_->mt_get_type(cxt_id, table_name)) {
      ...
      case MatchTableType::STATEFUL:
        get_entry_from_key_common<
                FlowTable, &RuntimeInterface::mt_st_get_entry_from_key>(
...
template <>
void
StandardHandler::build_action_entry<FlowTable::Entry>(
        BmActionEntry *action_e, const FlowTable::Entry &entry) {
  BmActionData action_data;
  for (const Data &d : entry.action_data.action_data) {
    action_data.push_back(d.get_string());
  }
