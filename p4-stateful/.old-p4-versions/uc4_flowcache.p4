...
control IngressPipeImpl (inout parsed_headers_t hdr,
    ...
    stateful_element stage_0 {
        flow_key = local_metadata.fk1; //from the parser --> (VLAN_ID, ip_dst)
        flow_cxt = {bit<32> state; bit<32> in_flight_cnt; bit<24> vxlan_vni;}
        idle_timeout = 30000;
        eviction_policy = LFU;
        size = 4096;
    }
    ...
    apply {
        if (standard_metadata.ingress_port == DC_NETWORK_PORT) {
            ...
            if (standard_metadata.ingress_port == OTR_0_PORT) {
                local_metadata.fk1 = ((bit<12>) hdr.vxlan.reserved2) | (hdr.inner_ipv4.dst_addr << 12);
            } else {
                local_metadata.fk1 = hdr.vlan.vid | (hdr.ipv4.dst_addr << 12);
            }
            ...
            stage_0.apply({
                if (standard_metadata.ingress_port == OTR_0_PORT) {
                    if (flow_ctx.state == NEW) {
                        ...
                        flow_ctx.state = LEARNT;
                    }
                    else if (flow_ctx.state == LEARNT)  {
                        flow_ctx.in_flight_cnt = flow_ctx.in_flight_cnt - 1;
                        standard_metadata.egress_spec = DC_NETWORK_PORT;
                    }
                }
                else if (standard_metadata.ingress_port != OTR_0_PORT) { // bare metal ports
                    if (flow_ctx.state == NEW) {
                        fwd_to_otr();
                        flow_ctx.in_flight_cnt = flow_ctx.in_flight_cnt + 1;
                    }
