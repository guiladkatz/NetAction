...
class SwitchConnection(object):
    ...
    def MasterArbitrationUpdate(self, election_low=2, role_id=2, dry_run=False, **kwargs):
        request = p4runtime_pb2.StreamMessageRequest()
        request.arbitration.device_id = self.device_id
        #request.arbitration.role.id = role_id
        request.arbitration.election_id.high = 0
        request.arbitration.election_id.low = election_low
    ...
    def SetForwardingPipelineConfig(self, p4info, election_low=2, role_id=2, dry_run=False, **kwargs):
        device_config = self.buildDeviceConfig(**kwargs)
        request = p4runtime_pb2.SetForwardingPipelineConfigRequest()
        request.election_id.low = election_low
    ...
    def WriteTableEntry(self, table_entry, election_low=2, role_id=2, dry_run=False):
    ...
    def WriteTableEntry_flowCache(self, table_entry, election_low=2, role_id=2, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = election_low
        request.role_id = role_id
        ...
        update.type = p4runtime_pb2.Update.INSERT
        update.entity.table_entry.CopyFrom(table_entry)
        update.entity.table_entry.idle_timeout_ns = 5000000000
        if dry_run:
            print "P4Runtime Write:", request
        else:
            self.client_stub.Write(request)
    ...
    # Modify 
    def ModifyTableEntry(self, table_entry, election_low=2, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = election_low
        update = request.updates.add()
    ...
    # Modify Counter 
    def ModifyDirectCounter(self, table_entry, election_low=2, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = election_low
        update = request.updates.add()
    ...
    # Delete 
    def DeleteTableEntry(self, table_entry, election_low=2, role_id=2, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = election_low
        request.role_id = role_id
    ...
    def ReadTableEntries(self, table_id=None, dry_run=False):
        ...
        table_entry.time_since_last_hit.elapsed_ns = 1
        table_entry.counter_data.packet_count = 1
    ...
    def ReadTableEntries_batch(self, table_entry_arr=None, table_id=None, dry_run=False):

        request = p4runtime_pb2.ReadRequest()
        request.device_id = self.device_id
        if table_entry_arr is not None:
            for table_entry in table_entry_arr:
                entity = request.entities.add()
                table_entry = entity.table_entry
                table_entry.time_since_last_hit.elapsed_ns = 1
                table_entry.counter_data.packet_count = 1
    ...
    def ReadDirectCounter(self, table_entry=None, table_id=None, dry_run=False):
        request = p4runtime_pb2.ReadRequest()
        request.device_id = self.device_id
        entity = request.entities.add()
        ...
        # assign
        direct_counter_entry.table_entry.CopyFrom(table_entry)
        direct_counter_entry.table_entry.time_since_last_hit.elapsed_ns = 1
        """
        if table_id is not None:
        ...
        if table_entry is not None:
            direct_counter_entry.table_entry.match = table_entry.match
        else:
            direct_counter_entry.table_entry.match = 0
        """
        if dry_run:
            ...
            for response in self.client_stub.Read(request):
                yield response
    def ReadDirectCounter_Batch(self, table_entry_arr=None, table_id=None, dry_run=False):
        request = p4runtime_pb2.ReadRequest()
        request.device_id = self.device_id
        if table_entry_arr is not None:
            for table_entry in table_entry_arr:
    ...
    def WritePRE(self, mc_group, election_low=2, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = election_low
        update = request.updates.add()
    ...
    # Digest 
    def WriteDigestEntry(self, digest_entry, election_low=2, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id 
        request.election_id.low = election_low
        update = request.updates.add()
